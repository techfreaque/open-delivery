...
export function usePaymentMethods() {
  ...
  const fetchPaymentMethods = async () => {
    setIsLoading(true);
    try {
      const paymentMethodsString = await AsyncStorage.getItem('user_payment_methods');
      if (paymentMethodsString) {
        const savedPaymentMethods = JSON.parse(paymentMethodsString) as PaymentMethod[];
  ...
  const addPaymentMethod = async (paymentMethod: Omit<PaymentMethod, 'id' | 'synced'>) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      await AsyncStorage.setItem('user_payment_methods', JSON.stringify(updatedPaymentMethods));
