...
export default async function seedTestDatabase(): Promise<void> {
  ...
  for (const restaurant of Object.values(restaurantExamples)) {
    try {
      const confirmedData = await getCoordinatesFromAddress(restaurant);
      if ("error" in confirmedData) {
        throw new Error(
          `Failed to get coordinates for restaurant ${restaurant.name}: ${confirmedData.error}`,
        );
      }
      const { latitude, longitude } = confirmedData;
