...
import { Clock, MapPin, Phone, MessageSquare, ChevronRight, X } from 'lucide-react-native';
    ...
    <View style={styles.activeOrderCard}>
      ...
      <View style={styles.deliveryPersonContainer}>
        ...
        <View style={styles.deliveryPersonInfo}>
          <Text style={styles.deliveryPersonName}>{order.deliveryPerson.name} is delivering your order</Text>
          <Text style={styles.deliveryPersonPhone}>{order.deliveryPerson.phone}</Text>
        </View>
  ...
  const renderCompletedOrder = (order) => (
    ...
    >
      <View style={styles.completedOrderHeader}>
        <Image source={{ uri: order.image }} style={styles.completedOrderImage} />
    ...
    <SafeAreaView style={styles.container}>
      ...
      {filteredOrders.length > 0 ? (
        <FlatList
          ...
          renderItem={({ item }) => 
            activeTab === 'active' ? renderActiveOrder(item) : renderCompletedOrder(item)
          }
        ...
        <View style={styles.emptyContainer}>
          <Image 
            source={{ uri: 'https://images.unsplash.com/photo-1594007654729-407eedc4fe24?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60' }} 
            ...
            <View style={styles.modalContent}>
              ...
              <View style={styles.orderDetailSection}>
                <View style={styles.orderDetailHeader}>
                  <Image source={{ uri: selectedOrder.image }} style={styles.orderDetailImage} />
                  <View>
                    <Text style={styles.orderDetailRestaurantName}>{selectedOrder.restaurant}</Text>
                    <Text style={styles.orderDetailDate}>{selectedOrder.orderDate}</Text>
              ...
              <View style={styles.orderDetailSection}>
                ...
                {selectedOrder.items.map((item, index) => (
                  <View key={index} style={styles.orderDetailItem}>
                    <Text style={styles.orderDetailItemName}>{item.quantity}x {item.name}</Text>
                  </View>
                ))}
                <View style={styles.orderDetailTotal}>
                  <Text style={styles.orderDetailTotalLabel}>Total</Text>
                  <Text style={styles.orderDetailTotalValue}>${selectedOrder.total.toFixed(2)}</Text>
                </View>
              </View>
              ...
              <View style={styles.orderDetailSection}>
                ...
                <View style={styles.orderDetailAddress}>
                  <MapPin size={16} color="#6B7280" />
                  <Text style={styles.orderDetailAddressText}>{selectedOrder.deliveryAddress}</Text>
                </View>
              </View>

              {selectedOrder.status === 'active' && selectedOrder.deliveryPerson && (
                <View style={styles.orderDetailSection}>
                  <Text style={styles.orderDetailSectionTitle}>Delivery Person</Text>
                  <View style={styles.deliveryPersonContainer}>
                    <Image source={{ uri: selectedOrder.deliveryPerson.image }} style={styles.deliveryPersonImage} />
                    <View style={styles.deliveryPersonInfo}>
                      <Text style={styles.deliveryPersonName}>{selectedOrder.deliveryPerson.name}</Text>
                      <Text style={styles.deliveryPersonPhone}>{selectedOrder.deliveryPerson.phone}</Text>
                    </View>
                    <View style={styles.contactButtons}>
                      <TouchableOpacity 
                        style={styles.contactButton}
                        onPress={() => handleCallDriver(selectedOrder.deliveryPerson.phone)}
                      >
                        <Phone size={20} color="#FF5A5F" />
                      </TouchableOpacity>
                      <TouchableOpacity 
                        style={styles.contactButton}
                        onPress={() => handleMessageDriver(selectedOrder.deliveryPerson.name)}
                      >
                        <MessageSquare size={20} color="#FF5A5F" />
                      </TouchableOpacity>
                    </View>
