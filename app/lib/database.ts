...
import { CartItem, Address, PaymentMethod, Order, MenuItem, Restaurant } from '../types';
...
const webStorage = {
  ...
  paymentMethods: [
    { id: '1', type: 'Visa', last4: '4242', is_default: 1 }
  ],
  cartItems: []
};
...
export const insertRestaurant = (restaurant: any): Promise<void> => {
  if (Platform.OS === 'web') {
    const index = webStorage.restaurants.findIndex(r => r.id === restaurant.id);
    if (index >= 0) {
...
export const getMenuItemsByRestaurantId = (restaurantId: string): Promise<any[]> => {
  if (Platform.OS === 'web') {
    return Promise.resolve(webStorage.menuItems.filter(m => m.restaurant_id === restaurantId));
  }
...
export const addToCart = (item: any): Promise<void> => {
  if (Platform.OS === 'web') {
    const index = webStorage.cartItems.findIndex(i => i.menu_item_id === item.menu_item_id);
    if (index >= 0) {
...
export const updateCartItemQuantity = (id: string, quantity: number): Promise<void> => {
  if (Platform.OS === 'web') {
    if (quantity <= 0) {
...
export const getAddresses = (): Promise<any[]> => {
  if (Platform.OS === 'web') {
    ...
    return Promise.resolve(webStorage.addresses.map(addr => ({
      ...addr,
...
export const insertPaymentMethod = (paymentMethod: any): Promise<void> => {
  if (Platform.OS === 'web') {
    ...
    const index = webStorage.paymentMethods.findIndex(p => p.id === paymentMethod.id);
    if (index >= 0) {
    ...
    db.transaction(tx => {
      ...
      if (paymentMethod.is_default) {
        tx.executeSql(
          ...
          (_, error) => {
            console.error('Error updating existing default payment methods:', error);
            reject(error);
            return false;
          }
        );
...
export const getPaymentMethods = (): Promise<any[]> => {
  if (Platform.OS === 'web') {
    // Convert SQLite integer to boolean for is_default
    return Promise.resolve(webStorage.paymentMethods.map(method => ({
      ...method,
...
export const deletePaymentMethod = (id: string): Promise<void> => {
  if (Platform.OS === 'web') {
    webStorage.paymentMethods = webStorage.paymentMethods.filter(p => p.id !== id);
    return Promise.resolve();
  }
...
const markAsSynced = (table: string, ids: string[]): Promise<void> => {
  if (ids.length === 0 || Platform.OS === 'web') return Promise.resolve();
