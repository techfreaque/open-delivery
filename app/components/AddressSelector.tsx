...
import { MapPin, Navigation, X, Check, ChevronRight, Plus, Search, CircleAlert as AlertCircle } from 'lucide-react-native';
...
export default function AddressSelector({
  ...
  const { addresses, addAddress, setDefaultAddress, deleteAddress, isLoading, error } = useAddresses();
  ...
  const handleGetCurrentLocation = async () => {
    ...
    try {
        ...
        Alert.alert(
          ...
          [
            ...
            },
            {
              text: "Yes",
              onPress: async () => {
                const success = await addAddress(address, addresses.length === 0);
                if (success) {
      ...
      setLocationError('Error getting your location. Please try entering your address manually.');
      setCustomAddressMode(true);
    } finally {
      setLocationLoading(false);
    }
  ...
  const formatGeocodedAddress = (geocode: Location.LocationGeocodedAddress): string => {
    const components = [
      geocode.name,
      geocode.street,
            ...
            {!customAddressMode ? (
                ...
                {locationPermissionDenied && (
                  <View style={styles.permissionInstructions}>
                    ...
                    <View style={styles.permissionSteps}>
                      ...
                      <Text style={styles.permissionStep}>2. Select "Site settings" or "Permissions"</Text>
                      <Text style={styles.permissionStep}>3. Enable "Location" permission</Text>
                      <Text style={styles.permissionStep}>4. Refresh the page and try again</Text>
                    </View>
                  </View>
                )}
                    ...
                    renderItem={({ item }) => (
                      ...
                      >
                        <View style={styles.addressItemContent}>
                          <MapPin
                            size={20}
                            color={selectedAddressId === item.id ? "#FFFFFF" : "#FF5A5F"}
                          />
                          <View style={styles.addressItemText}>
                            <Text
                              style={[
                                styles.addressItemLabel,
                                selectedAddressId === item.id && styles.selectedAddressItemText
                              ]}
                            >
                              {item.address}
                            </Text>
              ...
              <>
                ...
                <Text style={styles.addressInputHelp}>
                  Please enter your complete address including street, city, and postal code
                </Text>
